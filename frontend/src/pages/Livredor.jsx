...
function Livredor() {
    ...
    const fetchMessages = async () => {
        setLoading(true);
        try {
            const data = await guestbookService.getPublic(page, 5);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await guestbookService.postMessage(content, isPrivate);
            setContent('');
            setIsPrivate(false);
            setPage(1);
            fetchMessages();
    ...
    return (
        <>
            <Helmet>
                <title>Reservoir Rock | Livre d'or</title>
                <meta
                    name="description"
                    content="Reservoir Rock, page sur le livre d'or."
                />
                <link rel="canonical" href="https://resrock.fr/livredor" />
            </Helmet>
